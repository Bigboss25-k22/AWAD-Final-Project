...
export interface IEmailWorkflowRepository {
  ...
  findByGmailMessageId(userId: string, gmailMessageId: string): Promise<EmailWorkflowEntity | null>;
  findByUserAndStatus(userId: string, status: WorkflowStatus): Promise<EmailWorkflowEntity[]>;
  ...
  updateStatus(id: string, status: WorkflowStatus): Promise<EmailWorkflowEntity>;
